...
from tasks import Difficulty, ReviewCommentRecord, ReviewDecision, ReviewSubmission, TaskDefinition, clamp_open_score, extract_line, get_task, grade_single_comment, grade_submission, list_tasks, run_simulated_linter, summarize_task_catalog
try:
    ...
    class Environment:
        ...
        def reset(self, seed: int | None=None, episode_id: str | None=None, **kwargs: Any) -> Observation:
        ...
        def get_metadata(self) -> EnvironmentMetadata:
            return EnvironmentMetadata(name=self.__class__.__name__, description=f'{self.__class__.__name__} environment', version='1.0.0')

        def close(self) -> None:
            pass

    def openenv_create_app(env: type[Environment], action_cls: type[Action], observation_cls: type[Observation], env_name: str | None=None, max_concurrent_envs: int | None=None) -> FastAPI:
        ...
        async def reset_endpoint(payload: dict[str, Any] | None=None) -> dict[str, Any]:
            ...
            return {'observation': observation.model_dump(), 'reward': observation.reward, 'done': observation.done}
...
class FileSnapshot(BaseModel):
    ...
    highlighted_line: int | None = Field(default=None, description='Optional line number that the agent most recently focused on.')
...
class CodeReviewAction(Action):
    ...
    file_path: str | None = Field(default=None, description='File path to open. Required for view_file.')
    command: str | None = Field(default=None, description='Linter command to execute. Required for run_linter.')
    line_number: int | None = Field(default=None, ge=1, description='Line number on the currently opened file for review comments.')
    comment_text: str | None = Field(default=None, description='Comment body to attach to the current file and line.')
    decision: ReviewDecision | None = Field(default=None, description='Final review decision. Required for submit_review.')
...
class CodeReviewObservation(Observation):
    ...
    available_files: list[str] = Field(default_factory=list, description='Files included in the simulated pull request.')
    current_file: FileSnapshot | None = Field(default=None, description='Currently visible file content, if any.')
    linter_output: LinterRun | None = Field(default=None, description='Most recent linter execution result.')
    pull_request_status: PullRequestStatus = Field(..., description='Current review status, accumulated comments, and final grading output.')
    steps_remaining: int = Field(..., ge=0, description='Remaining step budget before the episode auto-terminates.')
...
class EnterpriseCodeReviewEnv(Environment):
    ...
    def reset(self, seed: Optional[int]=None, episode_id: Optional[str]=None, task_id: str | None=None, difficulty: str | None=None, **kwargs: Any) -> CodeReviewObservation:
        task = self._select_task(seed=seed, task_id=task_id, difficulty=difficulty)
        self._task = task
        self._state = EnterpriseCodeReviewState(episode_id=episode_id or str(uuid4()), step_count=0, task_id=task.task_id, current_file_path=None, viewed_files=[], executed_commands=[], comments=[], submitted_decision=None, review_completed=False, final_score=0.01)
        self._latest_linter_run = None
        self._action_counts = {}
        self._rewarded_files = set()
        self._rewarded_commands = set()
        self._comment_keys = set()
        return self._build_observation(message='Review session initialized. Inspect the changed files, run safe analysis commands, leave line comments, and submit a final review.', reward=clamp_open_score(0.0), done=False)
    ...
    def step(self, action: CodeReviewAction, timeout_s: Optional[float]=None, **kwargs: Any) -> CodeReviewObservation:
        try:
            ...
            return self._build_observation(message=err_msg, reward=clamp_open_score(0.01), done=True)

    def _step_internal(self, action: CodeReviewAction, timeout_s: Optional[float]=None, **kwargs: Any) -> CodeReviewObservation:
        task = self._require_task()
        if self._state.review_completed:
            return self._build_observation(message='This review is already complete. Call reset() to start a new task.', reward=clamp_open_score(-0.1), done=True)
        ...
        if action.action_type == ActionType.VIEW_FILE:
            ...
            message, reward = self._handle_add_comment(line_number=action.line_number or 1, comment_text=action.comment_text or '')
        else:
        ...
        if not done and self._state.step_count >= task.max_steps:
            ...
            message = f'{message} Step budget exhausted before a final review was submitted.'
    ...
    def get_metadata(self) -> EnvironmentMetadata:
        return EnvironmentMetadata(name='enterprise_code_review', description='Enterprise code review environment with typed PR observations, safe linter actions, deterministic graders, and trajectory-aware rewards.', version='1.0.0', author='OpenAI Codex')

    def _select_task(self, seed: int | None, task_id: str | None, difficulty: str | None) -> TaskDefinition:
        if task_id:
    ...
    def _handle_view_file(self, file_path: str) -> tuple[str, float]:
        ...
        if file_path not in task.files:
            return (f"File '{file_path}' is not part of this pull request. Avoid hallucinating files.", -0.2)
        ...
        message = f'Opened {file_path}. Review the implementation carefully.' if snippet else f'Opened {file_path}.'
    ...
    def _handle_run_linter(self, command: str) -> tuple[str, float]:
        ...
        self._latest_linter_run = LinterRun(command=command, success=success, output=output, findings=[LinterFinding(file_path=finding.file_path, line_number=finding.line_number, rule_id=finding.rule_id, severity=finding.severity, message=finding.message) for finding in findings])
    ...
    def _handle_add_comment(self, line_number: int, comment_text: str) -> tuple[str, float]:
        ...
        record = ReviewCommentRecord(file_path=current_file, line_number=line_number, comment_text=comment_text)
        graded = grade_single_comment(task, record)
        self._comment_keys.add(dedupe_key)
        self._state.comments.append(ReviewComment(file_path=current_file, line_number=line_number, comment_text=comment_text, quality_score=graded.score))
        if graded.score >= 0.85:
            ...
            reward = -0.04
        message = f'Comment added on {current_file}:{line_number}. Quality score={graded.score:.4f}.'
        return (message, reward)

    def _handle_submit_review(self, decision: ReviewDecision | None) -> tuple[str, float]:
        task = self._require_task()
        submission = ReviewSubmission(comments=tuple((ReviewCommentRecord(file_path=comment.file_path, line_number=comment.line_number, comment_text=comment.comment_text) for comment in self._state.comments)), decision=decision)
        ...
        message = f"Review submitted with decision '{(decision.value if decision else 'unknown')}'. Deterministic grader score={final_score:.4f}."
        return (message, reward)

    def _build_observation(self, message: str, reward: float, done: bool) -> CodeReviewObservation:
        task = self._require_task()
        current_file = None
        if self._state.current_file_path:
            content = task.files[self._state.current_file_path]
            current_file = FileSnapshot(file_path=self._state.current_file_path, content=content, total_lines=len(content.splitlines()), highlighted_line=self._task.primary_target_line if self._state.current_file_path == task.primary_target_file else None)
        pull_request_status = PullRequestStatus(pr_title=task.pr_title, pr_description=task.pr_description, changed_files=list(task.files.keys()), task_id=task.task_id, difficulty=task.difficulty.value, comments=list(self._state.comments), submitted_decision=self._state.submitted_decision, review_completed=self._state.review_completed, grader_score=self._state.final_score)
        return CodeReviewObservation(done=done, reward=reward, metadata={'episode_id': self._state.episode_id, 'task_summary': task.summary, 'task_catalog': summarize_task_catalog()}, task_id=task.task_id, task_title=task.title, message=message, available_files=list(task.files.keys()), current_file=current_file, linter_output=self._latest_linter_run, pull_request_status=pull_request_status, steps_remaining=max(0, task.max_steps - self._state.step_count))
...
def build_app() -> FastAPI:
    app = FastAPI(title='Enterprise Code Review OpenEnv API', version='1.0.0', description='Persistent HTTP API for the Enterprise Code Review OpenEnv task. Use /reset, /step, and /state to run full review trajectories.')
    ...
    def root() -> dict[str, Any]:
        env = app.state.env
        return {'status': 'ok', 'name': 'enterprise_code_review', 'description': env.get_metadata().description, 'docs_url': '/docs', 'endpoints': ['/reset', '/step', '/state', '/schema', '/metadata', '/health']}
    ...
    def schema() -> SchemaEnvelope:
        return SchemaEnvelope(action=CodeReviewAction.model_json_schema(), observation=CodeReviewObservation.model_json_schema(), state=EnterpriseCodeReviewState.model_json_schema())
    ...
    def mcp(request: dict[str, Any] | None=Body(default=None)) -> JsonRpcEnvelope:
        ...
        return JsonRpcEnvelope(id=request_id, error=JsonRpcErrorEnvelope(code=-32601, message='MCP methods are not implemented for this HTTP surface. Use the simulation endpoints /reset, /step, and /state.'))
    ...
    def reset(request: ResetPayload=Body(default_factory=ResetPayload)) -> ResetStepEnvelope:
        ...
        return ResetStepEnvelope(observation=observation, reward=observation.reward, done=observation.done)

    @app.post('/step')
    def step(request: StepPayload) -> ResetStepEnvelope:
        observation = app.state.env.step(request.action, timeout_s=request.timeout_s)
        return ResetStepEnvelope(observation=observation, reward=observation.reward, done=observation.done)
    return app
app = build_app()
